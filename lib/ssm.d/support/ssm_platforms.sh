...
get_compatible_platforms() {
	local domain_home platform platforms
	local comp_platforms platforms_dir
	local filename line
	...
	platforms_dir="${domain_home}/etc/ssm.d/platforms"
	while [ "${platform}" != "" ]; do
		plat_dist=${platform%%-*}
...
print_usage() {
	echo "\
usage: ssm platforms [<domainHome>]

Where:
<domainHome>	Domain home in which to look for platform configuration
		settings."
}
...
if [ "${domain_home}" = "" ]; then
	if [ "${SSM_DOMAIN_HOME}" != "" ]; then
		domain_home="${SSM_DOMAIN_HOME}"
	elif [ "${SSMDOMAINHOME}" != "" ]; then
		domain_home="${SSMDOMAINHOME}"
	elif [ "${SSM_USER_DOMAIN_HOME}" != "" ]; then
		domain_home="${SSM_USER_DOMAIN_HOME}"
	elif [ "${SSMUSERDOMAINHOME}" != "" ]; then
		domain_home="${SSMUSERDOMAINHOME}"
	elif [ "${SSM_SYSTEM_DOMAIN_HOME}" != "" ]; then
		domain_home="${SSM_SYSTEM_DOMAIN_HOME}"
	elif [ "${SSMSYSTEMDOMAINHOME}" != "" ]; then
		domain_home="${SSMSYSTEMDOMAINHOME}"
	else
		echo "error: cannot determine domain home automatically"
		exit 1
	fi
fi
...
target_dir=`cd "${domain_home}/lib/ssm.d"; pwd`
...
if [ "${self_dir}" = "${target_dir}" ]; then
	#get_system_platforms "${domain_home}"
	platform=`get_base_platform`
	get_compatible_platforms "${domain_home}" "${platform}"
else
	# call helper from domain
	if [ -r "${target_dir}/ssm_platforms.sh" ]; then
		exec "${target_dir}/ssm_platforms.sh" "${domain_home}"
	else
		# pre v9.0
		. ${target_dir}/ssm_lib.sh
		getSystemPlatforms "${domain_home}"
	fi
fi
