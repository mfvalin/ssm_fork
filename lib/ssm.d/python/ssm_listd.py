...
def get_output_records(domain_home, publish_platform, format_fields):
    ...
    try:
        if os.path.isdir(domain_home):
            ...
            published_packages = domain.get_packages_with_state("published", publish_platform)
            ...
            for package_name in domain.get_package_names(package_name_pattern):
                ...
                if package_name.endswith("_"+publish_platform) or domain.is_published(package_name, publish_platform):
    ...
    for i, domain_home in enumerate(domain_homes):
        ...
        for domain_home in subdomains+[domain_home]:
            try:
                ...
                platforms = set([name.split("_", 2)[-1] for name in package_names]+domain.get_published_platforms())
                for platform in sorted(platforms):
                    lines = []
                    for record in get_output_records(domain_home, platform, format_fields):
                        if output_type == "format":
                            lines.append(fmt % sep.join(record))
                        else:
                            lines.append(fmt % tuple(record))
                    if output_type == "summary":
                        print "==========  %s (%s) (%s)  ==========" % (domain_home, domain.get_label(), platform)
